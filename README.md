...
* **Endless** — открывается после победы над боссом. Волны генерируются
  бесконечно: сначала только `then` и `setTimeout`, дальше добавляются
  `queueMicrotask`, продолжения после `await`, `setInterval` и операции, которые
  во время выполнения ставят новые. Растёт не множитель, а словарь операций,
  который нужно удержать в голове. Три нарушения — конец забега; рекорд
  (волна и очки) хранится отдельно и в очки курса не подмешивается, иначе ранг
  зарабатывался бы гриндом.
...
| `endless/*.test.ts`                   | генератор волн (детерминизм, расширение словаря, конечность волны) и логика забега |
...
| `pages/*.test.tsx`                    | меню, экран уровня, итоги прохождения, бесконечный режим           |
...
├── endless/
│   ├── generator.ts   # процедурные волны бесконечного режима
│   └── run.ts         # состояние забега: волны, серия, жизни, счёт
...
├── pages/             # Menu, Game, Results, Endless
├── state/gameStore.tsx
